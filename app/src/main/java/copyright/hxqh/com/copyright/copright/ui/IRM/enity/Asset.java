package copyright.hxqh.com.copyright.copright.ui.IRM.enity;


import java.io.Serializable;
import java.util.List;

import copyright.hxqh.com.copyright.copright.entity.RightInfo;

public class Asset implements Serializable {

    private String cutoffdate;
    private String otherrestrict;
    private String isContinue;
    private String count;
    private String remark;
    private String createCity;
    private String createdate;
    private String orignid;
    private String resourcetype;
    private String rightgainwayatt;
    private String rightgainway;
    private String warehousestatus;
    private String departmentnum;
    private String postbelongs;
    private String authorid;
    private String isrelease;
    private String resmedium;
    private String createNationSel;
    private String ishaveagreement;
    private String credentials;
    private String rightgainnote;
    private String entrymode;
    private String rightgainnotelist;
    private String matterearnings;
    private String status;
    private String finishplace;
    private String resourcename;
    private String specialrestrict;
    private String authorname;
    private List<RightInfo> rightinfoList;
    private String departmentname;
    private String previewimg;
    private String cost;
    private String selectedResourcesList;
    private String fstrlsadd;
    private String rightearnings;
    private String cstartdate;
    private String createNation;
    private String createdateSort;
    private String rightinfoid;
    private String contactids;
    private String audvispdtid;
    private String putcount;
    private String resgainway;
    private int pagenum;
    private String purpose;
    private String resourcekind;
    private String isbnnumatt;
    private String detailid;
    private String pageTag;
    private List<Rchrrel> rchrrelList;
    private String statuslist;
    private String resfinishdate;
    private String righterid;
    private String resourceno;
    private String signname;
    private String isPutInStorage;
    private String fstrlsdate;
    private String searchdept;
    private String createCitySel;
    private String resourcelabel;
    private String isqueryofficeid;
    private String isresource;
    private String officeid;
    private String relevantcontract;
    private String resourcecost;
    private String belongway;
    private String isassets;
    private String iscontact;
    private String rightinfocount;
    private String cenddate;
    private String warningdate;
    private String costSort;
    private String publicNation;
    private String reslabel;
    private String resourcekindlist;
    private String isbnnum;
    private String isfstrls;
    private String rid;
    private String publicNationSel;
    private String countnum;
    private String ishavesigned;
    private String resdiscribe;
    private String reslevel;
    private String issecret;
    private String publicCity;
    private String publicCitySel;
    private String crownerids;
    private String iscutoff;
    private String releaseplace;
    private String createnature;
    private int resourcenum,assetnum;

    public int getResourcenum() {
        return resourcenum;
    }

    public void setResourcenum(int resourcenum) {
        this.resourcenum = resourcenum;
    }

    public int getAssetnum() {
        return assetnum;
    }

    public void setAssetnum(int assetnum) {
        this.assetnum = assetnum;
    }

    public void setCutoffdate(String cutoffdate) {
        this.cutoffdate = cutoffdate;
    }
    public String getCutoffdate() {
        return cutoffdate;
    }

    public void setOtherrestrict(String otherrestrict) {
        this.otherrestrict = otherrestrict;
    }
    public String getOtherrestrict() {
        return otherrestrict;
    }

    public void setIsContinue(String isContinue) {
        this.isContinue = isContinue;
    }
    public String getIsContinue() {
        return isContinue;
    }

    public void setCount(String count) {
        this.count = count;
    }
    public String getCount() {
        return count;
    }

    public void setRemark(String remark) {
        this.remark = remark;
    }
    public String getRemark() {
        return remark;
    }

    public void setCreateCity(String createCity) {
        this.createCity = createCity;
    }
    public String getCreateCity() {
        return createCity;
    }

    public void setCreatedate(String createdate) {
        this.createdate = createdate;
    }
    public String getCreatedate() {
        return createdate;
    }

    public void setOrignid(String orignid) {
        this.orignid = orignid;
    }
    public String getOrignid() {
        return orignid;
    }

    public void setResourcetype(String resourcetype) {
        this.resourcetype = resourcetype;
    }
    public String getResourcetype() {
        return resourcetype;
    }

    public void setRightgainwayatt(String rightgainwayatt) {
        this.rightgainwayatt = rightgainwayatt;
    }
    public String getRightgainwayatt() {
        return rightgainwayatt;
    }

    public void setRightgainway(String rightgainway) {
        this.rightgainway = rightgainway;
    }
    public String getRightgainway() {
        return rightgainway;
    }

    public void setWarehousestatus(String warehousestatus) {
        this.warehousestatus = warehousestatus;
    }
    public String getWarehousestatus() {
        return warehousestatus;
    }

    public void setDepartmentnum(String departmentnum) {
        this.departmentnum = departmentnum;
    }
    public String getDepartmentnum() {
        return departmentnum;
    }

    public void setPostbelongs(String postbelongs) {
        this.postbelongs = postbelongs;
    }
    public String getPostbelongs() {
        return postbelongs;
    }

    public void setAuthorid(String authorid) {
        this.authorid = authorid;
    }
    public String getAuthorid() {
        return authorid;
    }

    public void setIsrelease(String isrelease) {
        this.isrelease = isrelease;
    }
    public String getIsrelease() {
        return isrelease;
    }

    public void setResmedium(String resmedium) {
        this.resmedium = resmedium;
    }
    public String getResmedium() {
        return resmedium;
    }

    public void setCreateNationSel(String createNationSel) {
        this.createNationSel = createNationSel;
    }
    public String getCreateNationSel() {
        return createNationSel;
    }

    public void setIshaveagreement(String ishaveagreement) {
        this.ishaveagreement = ishaveagreement;
    }
    public String getIshaveagreement() {
        return ishaveagreement;
    }

    public void setCredentials(String credentials) {
        this.credentials = credentials;
    }
    public String getCredentials() {
        return credentials;
    }

    public void setRightgainnote(String rightgainnote) {
        this.rightgainnote = rightgainnote;
    }
    public String getRightgainnote() {
        return rightgainnote;
    }

    public void setEntrymode(String entrymode) {
        this.entrymode = entrymode;
    }
    public String getEntrymode() {
        return entrymode;
    }

    public void setRightgainnotelist(String rightgainnotelist) {
        this.rightgainnotelist = rightgainnotelist;
    }
    public String getRightgainnotelist() {
        return rightgainnotelist;
    }

    public void setMatterearnings(String matterearnings) {
        this.matterearnings = matterearnings;
    }
    public String getMatterearnings() {
        return matterearnings;
    }

    public void setStatus(String status) {
        this.status = status;
    }
    public String getStatus() {
        return status;
    }

    public void setFinishplace(String finishplace) {
        this.finishplace = finishplace;
    }
    public String getFinishplace() {
        return finishplace;
    }

    public void setResourcename(String resourcename) {
        this.resourcename = resourcename;
    }
    public String getResourcename() {
        return resourcename;
    }

    public void setSpecialrestrict(String specialrestrict) {
        this.specialrestrict = specialrestrict;
    }
    public String getSpecialrestrict() {
        return specialrestrict;
    }

    public void setAuthorname(String authorname) {
        this.authorname = authorname;
    }
    public String getAuthorname() {
        return authorname;
    }

    public void setRightinfoList(List<RightInfo> rightinfoList) {
        this.rightinfoList = rightinfoList;
    }
    public List<RightInfo> getRightinfoList() {
        return rightinfoList;
    }

    public void setDepartmentname(String departmentname) {
        this.departmentname = departmentname;
    }
    public String getDepartmentname() {
        return departmentname;
    }

    public void setPreviewimg(String previewimg) {
        this.previewimg = previewimg;
    }
    public String getPreviewimg() {
        return previewimg;
    }

    public void setCost(String cost) {
        this.cost = cost;
    }
    public String getCost() {
        return cost;
    }

    public void setSelectedResourcesList(String selectedResourcesList) {
        this.selectedResourcesList = selectedResourcesList;
    }
    public String getSelectedResourcesList() {
        return selectedResourcesList;
    }

    public void setFstrlsadd(String fstrlsadd) {
        this.fstrlsadd = fstrlsadd;
    }
    public String getFstrlsadd() {
        return fstrlsadd;
    }

    public void setRightearnings(String rightearnings) {
        this.rightearnings = rightearnings;
    }
    public String getRightearnings() {
        return rightearnings;
    }

    public void setCstartdate(String cstartdate) {
        this.cstartdate = cstartdate;
    }
    public String getCstartdate() {
        return cstartdate;
    }

    public void setCreateNation(String createNation) {
        this.createNation = createNation;
    }
    public String getCreateNation() {
        return createNation;
    }

    public void setCreatedateSort(String createdateSort) {
        this.createdateSort = createdateSort;
    }
    public String getCreatedateSort() {
        return createdateSort;
    }

    public void setRightinfoid(String rightinfoid) {
        this.rightinfoid = rightinfoid;
    }
    public String getRightinfoid() {
        return rightinfoid;
    }

    public void setContactids(String contactids) {
        this.contactids = contactids;
    }
    public String getContactids() {
        return contactids;
    }

    public void setAudvispdtid(String audvispdtid) {
        this.audvispdtid = audvispdtid;
    }
    public String getAudvispdtid() {
        return audvispdtid;
    }

    public void setPutcount(String putcount) {
        this.putcount = putcount;
    }
    public String getPutcount() {
        return putcount;
    }

    public void setResgainway(String resgainway) {
        this.resgainway = resgainway;
    }
    public String getResgainway() {
        return resgainway;
    }

    public void setPagenum(int pagenum) {
        this.pagenum = pagenum;
    }
    public int getPagenum() {
        return pagenum;
    }

    public void setPurpose(String purpose) {
        this.purpose = purpose;
    }
    public String getPurpose() {
        return purpose;
    }

    public void setResourcekind(String resourcekind) {
        this.resourcekind = resourcekind;
    }
    public String getResourcekind() {
        return resourcekind;
    }

    public void setIsbnnumatt(String isbnnumatt) {
        this.isbnnumatt = isbnnumatt;
    }
    public String getIsbnnumatt() {
        return isbnnumatt;
    }

    public void setDetailid(String detailid) {
        this.detailid = detailid;
    }
    public String getDetailid() {
        return detailid;
    }

    public void setPageTag(String pageTag) {
        this.pageTag = pageTag;
    }
    public String getPageTag() {
        return pageTag;
    }

    public void setRchrrelList(List<Rchrrel> rchrrelList) {
        this.rchrrelList = rchrrelList;
    }
    public List<Rchrrel> getRchrrelList() {
        return rchrrelList;
    }

    public void setStatuslist(String statuslist) {
        this.statuslist = statuslist;
    }
    public String getStatuslist() {
        return statuslist;
    }

    public void setResfinishdate(String resfinishdate) {
        this.resfinishdate = resfinishdate;
    }
    public String getResfinishdate() {
        return resfinishdate;
    }

    public void setRighterid(String righterid) {
        this.righterid = righterid;
    }
    public String getRighterid() {
        return righterid;
    }

    public void setResourceno(String resourceno) {
        this.resourceno = resourceno;
    }
    public String getResourceno() {
        return resourceno;
    }

    public void setSignname(String signname) {
        this.signname = signname;
    }
    public String getSignname() {
        return signname;
    }

    public void setIsPutInStorage(String isPutInStorage) {
        this.isPutInStorage = isPutInStorage;
    }
    public String getIsPutInStorage() {
        return isPutInStorage;
    }

    public void setFstrlsdate(String fstrlsdate) {
        this.fstrlsdate = fstrlsdate;
    }
    public String getFstrlsdate() {
        return fstrlsdate;
    }

    public void setSearchdept(String searchdept) {
        this.searchdept = searchdept;
    }
    public String getSearchdept() {
        return searchdept;
    }

    public void setCreateCitySel(String createCitySel) {
        this.createCitySel = createCitySel;
    }
    public String getCreateCitySel() {
        return createCitySel;
    }

    public void setResourcelabel(String resourcelabel) {
        this.resourcelabel = resourcelabel;
    }
    public String getResourcelabel() {
        return resourcelabel;
    }

    public void setIsqueryofficeid(String isqueryofficeid) {
        this.isqueryofficeid = isqueryofficeid;
    }
    public String getIsqueryofficeid() {
        return isqueryofficeid;
    }

    public void setIsresource(String isresource) {
        this.isresource = isresource;
    }
    public String getIsresource() {
        return isresource;
    }

    public void setOfficeid(String officeid) {
        this.officeid = officeid;
    }
    public String getOfficeid() {
        return officeid;
    }

    public void setRelevantcontract(String relevantcontract) {
        this.relevantcontract = relevantcontract;
    }
    public String getRelevantcontract() {
        return relevantcontract;
    }

    public void setResourcecost(String resourcecost) {
        this.resourcecost = resourcecost;
    }
    public String getResourcecost() {
        return resourcecost;
    }

    public void setBelongway(String belongway) {
        this.belongway = belongway;
    }
    public String getBelongway() {
        return belongway;
    }

    public void setIsassets(String isassets) {
        this.isassets = isassets;
    }
    public String getIsassets() {
        return isassets;
    }

    public void setIscontact(String iscontact) {
        this.iscontact = iscontact;
    }
    public String getIscontact() {
        return iscontact;
    }

    public void setRightinfocount(String rightinfocount) {
        this.rightinfocount = rightinfocount;
    }
    public String getRightinfocount() {
        return rightinfocount;
    }

    public void setCenddate(String cenddate) {
        this.cenddate = cenddate;
    }
    public String getCenddate() {
        return cenddate;
    }

    public void setWarningdate(String warningdate) {
        this.warningdate = warningdate;
    }
    public String getWarningdate() {
        return warningdate;
    }

    public void setCostSort(String costSort) {
        this.costSort = costSort;
    }
    public String getCostSort() {
        return costSort;
    }

    public void setPublicNation(String publicNation) {
        this.publicNation = publicNation;
    }
    public String getPublicNation() {
        return publicNation;
    }

    public void setReslabel(String reslabel) {
        this.reslabel = reslabel;
    }
    public String getReslabel() {
        return reslabel;
    }

    public void setResourcekindlist(String resourcekindlist) {
        this.resourcekindlist = resourcekindlist;
    }
    public String getResourcekindlist() {
        return resourcekindlist;
    }

    public void setIsbnnum(String isbnnum) {
        this.isbnnum = isbnnum;
    }
    public String getIsbnnum() {
        return isbnnum;
    }

    public void setIsfstrls(String isfstrls) {
        this.isfstrls = isfstrls;
    }
    public String getIsfstrls() {
        return isfstrls;
    }

    public void setRid(String rid) {
        this.rid = rid;
    }
    public String getRid() {
        return rid;
    }

    public void setPublicNationSel(String publicNationSel) {
        this.publicNationSel = publicNationSel;
    }
    public String getPublicNationSel() {
        return publicNationSel;
    }

    public void setCountnum(String countnum) {
        this.countnum = countnum;
    }
    public String getCountnum() {
        return countnum;
    }

    public void setIshavesigned(String ishavesigned) {
        this.ishavesigned = ishavesigned;
    }
    public String getIshavesigned() {
        return ishavesigned;
    }

    public void setResdiscribe(String resdiscribe) {
        this.resdiscribe = resdiscribe;
    }
    public String getResdiscribe() {
        return resdiscribe;
    }

    public void setReslevel(String reslevel) {
        this.reslevel = reslevel;
    }
    public String getReslevel() {
        return reslevel;
    }

    public void setIssecret(String issecret) {
        this.issecret = issecret;
    }
    public String getIssecret() {
        return issecret;
    }

    public void setPublicCity(String publicCity) {
        this.publicCity = publicCity;
    }
    public String getPublicCity() {
        return publicCity;
    }

    public void setPublicCitySel(String publicCitySel) {
        this.publicCitySel = publicCitySel;
    }
    public String getPublicCitySel() {
        return publicCitySel;
    }

    public void setCrownerids(String crownerids) {
        this.crownerids = crownerids;
    }
    public String getCrownerids() {
        return crownerids;
    }

    public void setIscutoff(String iscutoff) {
        this.iscutoff = iscutoff;
    }
    public String getIscutoff() {
        return iscutoff;
    }

    public void setReleaseplace(String releaseplace) {
        this.releaseplace = releaseplace;
    }
    public String getReleaseplace() {
        return releaseplace;
    }

    public void setCreatenature(String createnature) {
        this.createnature = createnature;
    }
    public String getCreatenature() {
        return createnature;
    }

}
...
import copyright.hxqh.com.copyright.copright.ui.statistics.entity.ProductSerie;
...
public class ProductCostingActivity extends FragmentActivity {
    ...
    ProductCostAdapter productCostAdapter;
    ...
    //y轴的数据
    ArrayList<Float> cost_itemY = new ArrayList<Float>();
    ArrayList<Float> earn_itemY = new ArrayList<Float>();

    ArrayList<Integer> earns_data = new ArrayList<>();
    ArrayList<Integer> cost_data = new ArrayList<>();

    ArrayList<String> text3 = new ArrayList<>();
    List<Float> mRatios3 = new ArrayList<>();
    ArrayList<Integer> cost_itemY3 = new ArrayList<Integer>();
    ArrayList<Integer> earn_itemY3 = new ArrayList<Integer>();


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_productcost);
    ...
    private View.OnClickListener imag_downOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (startAnaim()) {
                ...
                productCostAdapter = new ProductCostAdapter(ProductCostingActivity.this, earns_data,cost_data, text, (ArrayList<Float>) mRatios);
                recyclerView.setAdapter(productCostAdapter);
            } else {
                productCostAdapter = new ProductCostAdapter(ProductCostingActivity.this,earn_itemY3,cost_itemY3,text3, (ArrayList<Float>) mRatios3);
                recyclerView.setAdapter(productCostAdapter);
    ...
    private View.OnClickListener btnlistener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (i == R.id.text_sure) {
                ...
                json = HttpConnect.getProductCostJson(ProductCostingActivity.this,textStarttime.getText().toString(),textEndtime.getText().toString(),"bar");
                getData(json);
    ...
    private void getData(final JSONObject json) {
        final String url = "http://118.190.115.150:8889/jeesite/htjk/apphttpjk/productCosting";
        new AsyncTask<String, String, ProductCost>() {

            @Override
            protected ProductCost doInBackground(String... strings) {
                ProductCost products =  HttpConnect.getProductCosting(json,url);
                return products;
            }
            @Override
            protected void onPostExecute(ProductCost preoducts) {
                super.onPostExecute(preoducts);
                if (preoducts!=null){
                    for (int i=0;i<preoducts.getSeries().size();i++){
                        if (preoducts.getSeries().get(i).getName().contains("产品成本")){
                            cost_data = preoducts.getSeries().get(i).getData();
                        }else if (preoducts.getSeries().get(i).getName().contains("产品收益")){
                            earns_data = preoducts.getSeries().get(i).getData();
                        }
                    }
                    text = (ArrayList<String>) preoducts.getXaxis().get(0).getData();
                }
                if (cost_data == null && earns_data == null){
                    linerLbar.setEnabled(false);
                    linerPie.setEnabled(false);
                    pieChartView.setVisibility(View.GONE);
                    liner_detail.setVisibility(View.GONE);
                    productCostAdapter = new ProductCostAdapter(ProductCostingActivity.this, earns_data,cost_data,text, (ArrayList<Float>) mRatios3);
                    recyclerView.setAdapter(productCostAdapter);
                }else {
    ...
    private void initPieDatas() {
        ...
        if (cost_data !=null && earns_data!=null){
            float sum = 0;
            for (int j = 0;j<cost_data.size();j++){
...
//                itemX.add(text.get(i).replaceAll("\r\n",""));
                ...
                if (Float.valueOf(cost_data.get(i)) == 0 || Float.valueOf(cost_data.get(i)) == 0.0){
                    ...
                    unstorage = Float.valueOf(String.format("%.4f",(float)Float.valueOf(cost_data.get(i))/sum));
                }
                mRatios.add(unstorage);
                mArcColors.add(ChartUtils.COLORS[i]);
            }
            for (int j = 0;j<earns_data.size();j++){
                earn_itemY.add(Float.valueOf(earns_data.get(j)));
            }
        }else {
            Toast.makeText(ProductCostingActivity.this, "暂无数据", Toast.LENGTH_SHORT).show();
        }
        if (earn_itemY.size() < 3){
            liner_detail.setVisibility(View.GONE);
            productCostAdapter = new ProductCostAdapter(this, earns_data,cost_data,text, (ArrayList<Float>) mRatios);
            recyclerView.setAdapter(productCostAdapter);
        }else {
            cost_itemY3.clear();
            earn_itemY3.clear();
            ...
            mRatios3.clear();
            for (int i=0;i<4;i++){
                cost_itemY3.add(cost_data.get(i));
                earn_itemY3.add(earns_data.get(i));
                mRatios3.add(mRatios.get(i));
                text3.add(itemX.get(i));
            }
            productCostAdapter = new ProductCostAdapter(this,earn_itemY3,cost_itemY3,text3, (ArrayList<Float>) mRatios3);
            recyclerView.setAdapter(productCostAdapter);
        }
        //点击动画开启
    ...
    private void initColumnDatas() {
        ...
        for (int i = 0; i < numColumns; ++i) {
            ...
            for (int j = 0; j < numSubcolumns; ++j) {
                values.add(new SubcolumnValue(cost_itemY.get(i), Color.rgb(39,71,132)));
                values.add(new SubcolumnValue(earn_itemY.get(i), Color.rgb(1,191,157)));
            }
